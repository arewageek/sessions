...
contract Sessions is ISessions, ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => uint256)) public balances; // videoId => user => balance
    ...
    modifier isOriginalVideo(uint _videoId) {
        require(!videos[_videoId].isFannit, "Not an original");
        _;
    }
    ...
    ) external override onlyCreator(_videoId) isOriginalVideo(_videoId) {
    ...
    function mintVideo(
        ...
        require(!video.isFannit, "Use mintFannit for fannits");

        bool success = _handleMint(_videoId, 0);
    ...
    function likeVideo(
        uint256 _videoId
    ) external override videoExists(_videoId) nonReentrant {
    ...
    ) external override videoExists(_videoId) {
    ...
    function tipCreator(
        ...
        _safeTransfer(_creator, msg.value, "Failed to tip creator");
    ...
    function getFannitMintLimit() external view override returns (uint256) {
        return fannitMintLimit;
    }
    ...
    function setFannitMintLimit(uint256 _limit) external override onlyOwner {
        ...
        fannitMintLimit = _limit;
        emit FannitMintLimitUpdated(_limit);
    }

    function setRefannitMintLimit(uint256 _limit) external override onlyOwner {
        require(_limit > 0, "Invalid limit");
        refannitMintLimit = _limit;
        emit RefannitMintLimitUpdated(_limit);
    }
     ...
     */
    function updateMaximumMintPrice(
        uint256 _newMaxMintPrice
    ) external onlyOwner {
        require(
            _newMaxMintPrice > 0 && _newMaxMintPrice != maxMintPrice,
            "Invalid Mint fee"
        );
     ...
     */
    function _validateNewOriginalParams(
        string memory _ipfsHash,
        uint256 _mintLimit,
        uint256 _priceInWei
    ) internal view {
     ...
     */
    function _validateFannitCreation(
        uint256 _originalVideoId,
        address _fan
    ) internal view {
        require(
            videos[_originalVideoId].creator != address(0),
            "Original doesn't exist"
        );
        require(!videos[_originalVideoId].isFannit, "Cannot fannit a fannit");
        require(
            !hasfannited[_fan][_originalVideoId],
     ...
     */
    function _createFannit(uint256 _parentVideoId, address _fan) internal {
        Video storage video = videos[_parentVideoId];
    ...
    /**
     * Get video length
     * @notice 0 => Vide is original
     * @notice 1 => fannit
     * @notice 2 => fannit of fannit
     */
    ...
    function _payShare(
        ...
        uint256 shareAmount = (_amount * _percentage) / 100;
    ...
    function _handleMint(uint256 _videoId, uint8 _videoDepth) internal returns (bool) {
        ...
        require(video.totalMints < video.mintLimit, "Sold out");
